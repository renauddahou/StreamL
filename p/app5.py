    ...
    if con:
        ...
        if result:
            ...
            if task == "":
                ...
                with col1:
                    if genre=="Python pour les enfants":
                        choice = st.selectbox("",elem)
                        ch=choice
                        if choice== ch:
                            with open('EXO/'+ch+'.py') as f:
                                Ex0 = f.read()
                            exo=st.code(Ex0, language = 'python')
                        

                        if button1:
                            st.write("**Solution"+" "+choice+"👇**\n"+"Run this code in the compiler below")
                            with open('SOL/'+ch+'.py') as f:
                                Ex0 = f.read()
                            exo=st.code(Ex0, language = 'python')
                        ...
                        #if button2:
                        elem2=[random.choice(elem)]
                        choice2 = st.selectbox("",elem2,key=1)
                        ch2=choice2
                        if choice2== ch2:
                            with open('EXO/'+ch2+'.py') as f:
                                Ex0 = f.read()
                            exo=st.code(Ex0, language = 'python')
                        if button3:
                            st.write("**Solution"+" "+choice2+"👇**\n"+"Run this code in the compiler below")
                            with open('SOL/'+ch2+'.py') as f:
                                Ex0 = f.read()
                            exo=st.code(Ex0, language = 'python')
                ...
                with col1:
                    st.markdown("**Exécutez votre code dans la notebook Jupyter pour plus d'expérience.** 👉 👉 👉[![Foo](https://jupyter.org/assets/logos/rectanglelogo-greytext-orangebody-greymoons.svg)](https://jupyter.org/try)")
